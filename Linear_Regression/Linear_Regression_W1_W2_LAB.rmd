...
does there seem to be a linear relationship? Let's take a look at the scatter 
plot between `homeruns` and `runs`.
</div>
```{r runs-vs-sth-else}
...
The scatter plot seems to suggest a linear relation between `homeruns` and `runs`. Let's keep going and do further analysis. Before fitting the data into a linear model, let's check the two variables correlation.

```{r}
mlb11 %>%
    summarise(cor(runs,homeruns))
```

The correlation suggests a fairly strong linear correlation between the two. Next, we fit the data into a linear model. 
...
Then, take a look at the scatterplot with the least squares line for m2 laid on top.
...
Now, let's check if applying a linear model is appropriate. First, let's consider 
its linearity by observing its residual plot:
...
ggplot(data=m2, aes(x=.fitted,y=.resid))+
    ...
    ylab('residuals')
```
From the plot, the residual is distributed around evenly around $0$ which 
indicates a linear relation. Then we check the distribution of the residual.
```{r}
...
The histogram plot is fairlly symmetric with center at $0$ shaped like a bell. 
Therefore, we can assume our data set satisfies the conditions for a linear regression.
